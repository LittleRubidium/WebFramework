...
func (ctx *Context) IJsonp(obj interface{}) IResponse {
	...
	//输出到前端页面的时候需要注意下进行字符过滤，否则有可能引起xss攻击
...
//Cookie
func (ctx *Context) ISetCookie(key, val string, maxAge int, path string, domain string, secure bool, httpOnly bool) IResponse {
	if path == "" {
		path = "/"
	}
